...
import { useOrders, useDesignOrders } from '../hooks/useFirebaseData';
import { Order, OrderStatus, DesignOrderStatus } from '../types';
...
const OrderManagement: React.FC = () => {
  ...
  // REPLACE DIRECT MOCK WITH HOOK
  const { orders, updateOrder, deleteOrder: deleteOrderHook } = useOrders();
  const { addDesignOrder } = useDesignOrders(); // ADD THIS
  ...
  const [viewOnly, setViewOnly] = useState(false); // ADD THIS for "View Details"
  ...
  const handleSubmitDesign = async () => {
    ...
    const newDesignOrder = {
      id: `DH${Date.now().toString().slice(-6)}`, // Generate Design Order ID
      customerName: selectedOrder.customerName,
      phone: selectedOrder.customerPhone,
      productType: selectedOrder.productName,
      requirements: designReq,
    ...
    await addDesignOrder(newDesignOrder);
    alert(`Đã tạo yêu cầu thiết kế thành công!`);
  ...
  const handleOpenEdit = (order: Order, isViewOnly: boolean = false) => {
    setSelectedOrder(order);
    setEditForm({ ...order }); // Copy order to edit form
  ...
  const handleSubmitEdit = async () => {
    if (!selectedOrder || !editForm.id) return;
  ...
  return (
    <div className="p-6 h-screen overflow-y-auto flex flex-col">
                ...
                <tr key={order.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="p-4 text-right relative action-menu">
                    <button
                      ...
                      className={`p-2 rounded-full transition-colors ${activeDropdown === order.id
      ...
      {isEditModalOpen && selectedOrder && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-[600px] max-w-[95vw] p-6 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold flex items-center gap-2">
                {viewOnly ? <Eye className="text-blue-600" /> : <Edit className="text-blue-600" />}
                {viewOnly ? 'Xem chi tiết đơn hàng' : 'Cập nhật thông tin đơn hàng'}
              </h3>
              <div className="px-3 py-1 bg-slate-100 rounded text-sm font-mono text-slate-500">
                {selectedOrder.id}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded focus:border-accent outline-none disabled:bg-slate-100 disabled:text-slate-500"
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Doanh thu dự kiến</label>
                <input
                  type="number"
                  disabled={viewOnly}
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 h-24 border border-slate-300 rounded focus:border-accent outline-none resize-none disabled:bg-slate-100 disabled:text-slate-500"
              />
            </div>
