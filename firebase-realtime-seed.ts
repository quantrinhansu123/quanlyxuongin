// Script seed dữ liệu vào Firebase Realtime Database
// Chạy: npx tsx firebase-realtime-seed.ts
...
const firebaseConfig = {
  ...
  databaseURL: "https://quanlyxuongin-d18cc-default-rtdb.asia-southeast1.firebasedatabase.app",
...
async function seedRealtimeDatabase() {
  try {
    ...
    if (firebaseConfig.apiKey === "YOUR_API_KEY") {
      ...
      console.error('📝 Vui lòng cập nhật firebaseConfig trong file này');
    ...
    await set(ref(db, 'nhom_khach_hang'), nhomKhachHang);
    ...
    await set(ref(db, 'nguon_khach_hang'), nguonKhachHang);
    ...
    await set(ref(db, 'nhan_vien'), nhanVien);
    ...
    await set(ref(db, 'nhom_san_pham'), nhomSanPham);
    ...
    await set(ref(db, 'san_pham'), sanPham);
    ...
    await set(ref(db, 'phan_bo_ban_hang'), phanBoBanHang);
    ...
    await set(ref(db, 'khach_hang'), khachHang);
    ...
    await set(ref(db, 'don_hang'), donHang);
    ...
    await set(ref(db, 'lich_su_thanh_toan'), lichSuThanhToan);
    ...
    await set(ref(db, 'mau_thiet_ke'), mauThietKe);
    ...
    await set(ref(db, 'don_thiet_ke'), donThietKe);
    ...
    console.error('  2. Realtime Database đã được bật trong Firebase Console?');
