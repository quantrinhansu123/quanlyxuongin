// Script để seed dữ liệu vào Firestore
// Chạy: npx tsx firebase-seed.ts hoặc node firebase-seed.js (sau khi compile)
...
const now = new Date();
const minutesAgo = (mins: number) => Timestamp.fromDate(new Date(now.getTime() - mins * 60000));
const daysFromNow = (days: number) => Timestamp.fromDate(new Date(now.getTime() + days * 86400000));

async function seedDatabase() {
  try {
    // Kiểm tra config
    if (firebaseConfig.apiKey === "YOUR_API_KEY" || firebaseConfig.projectId === "YOUR_PROJECT_ID") {
      console.error('❌ LỖI: Chưa cấu hình Firebase!');
      console.error('📝 Vui lòng cập nhật firebaseConfig trong file này với thông tin từ Firebase Console');
      console.error('🔗 Lấy config tại: Firebase Console > Project Settings > Your apps > Web app');
      process.exit(1);
    }
    ...
    const nhomKhachHang = [
      { id: 'NH01', ten: 'Ban le', mo_ta: 'Khách hàng bán lẻ', created_at: Timestamp.now() },
      { id: 'NH02', ten: 'Dai ly', mo_ta: 'Đại lý', created_at: Timestamp.now() },
      { id: 'NH03', ten: 'Du an', mo_ta: 'Dự án', created_at: Timestamp.now() },
      { id: 'NH04', ten: 'Vang lai', mo_ta: 'Vãng lai', created_at: Timestamp.now() }
    ];
    for (const item of nhomKhachHang) {
      await setDoc(doc(db, 'nhom_khach_hang', item.id), item);
      console.log(`  ✓ Đã tạo: ${item.id} - ${item.ten}`);
    }
    console.log(`✅ Hoàn thành: ${nhomKhachHang.length} nhóm khách hàng\n`);

    // 2. Seed Nguồn khách hàng
    console.log('Đang seed nguon_khach_hang...');
    const nguonKhachHang = [
      { id: 'NG01', ten: 'Facebook Ads', mo_ta: 'Quảng cáo Facebook', created_at: Timestamp.now() },
      { id: 'NG02', ten: 'Facebook Group', mo_ta: 'Nhóm Facebook', created_at: Timestamp.now() },
      { id: 'NG03', ten: 'Website', mo_ta: 'Website công ty', created_at: Timestamp.now() },
      { id: 'NG04', ten: 'Zalo', mo_ta: 'Zalo', created_at: Timestamp.now() },
      { id: 'NG05', ten: 'Gioi thieu', mo_ta: 'Giới thiệu', created_at: Timestamp.now() }
    ];
    for (const item of nguonKhachHang) {
      await setDoc(doc(db, 'nguon_khach_hang', item.id), item);
      console.log(`  ✓ Đã tạo: ${item.id} - ${item.ten}`);
    }
    console.log(`✅ Hoàn thành: ${nguonKhachHang.length} nguồn khách hàng\n`);
    ...
    const nhanVien = [
      {
        ...
        ngay_vao_lam: Timestamp.fromDate(new Date('2023-01-15')),
      ...
      },
      {
        ...
        ngay_vao_lam: Timestamp.fromDate(new Date('2023-03-20')),
        trang_thai: 'Dang lam viec',
        avatar: 'TB',
        created_at: Timestamp.now(),
        updated_at: Timestamp.now()
      },
      {
        ...
        ngay_vao_lam: Timestamp.fromDate(new Date('2022-05-10')),
      ...
      },
      {
        ...
        ngay_vao_lam: Timestamp.fromDate(new Date('2023-08-01')),
        trang_thai: 'Dang lam viec',
        avatar: 'PD',
        created_at: Timestamp.now(),
        updated_at: Timestamp.now()
      },
      {
        ...
        ngay_vao_lam: Timestamp.fromDate(new Date('2021-12-01')),
        trang_thai: 'Dang lam viec',
        avatar: 'HE',
        created_at: Timestamp.now(),
        updated_at: Timestamp.now()
    ...
    for (const item of nhanVien) {
      await setDoc(doc(db, 'nhan_vien', item.id), item);
      console.log(`  ✓ Đã tạo: ${item.id} - ${item.ten}`);
    }
    console.log(`✅ Hoàn thành: ${nhanVien.length} nhân viên\n`);

    // 4. Seed Nhóm sản phẩm
    console.log('Đang seed nhom_san_pham...');
    const nhomSanPham = [
      { id: 'NSP01', ten: 'Gia dung', mo_ta: 'Gia dụng', created_at: Timestamp.now() },
      { id: 'NSP02', ten: 'Dien tu', mo_ta: 'Điện tử', created_at: Timestamp.now() },
      { id: 'NSP03', ten: 'Cong nghiep', mo_ta: 'Công nghiệp', created_at: Timestamp.now() }
    ];
    for (const item of nhomSanPham) {
      await setDoc(doc(db, 'nhom_san_pham', item.id), item);
    }
    ...
    for (const item of sanPham) {
      await setDoc(doc(db, 'san_pham', item.id), item);
    }
    ...
    for (const item of phanBoBanHang) {
      await setDoc(doc(db, 'phan_bo_ban_hang', item.id), item);
    }
    ...
    for (const item of khachHang) {
      await setDoc(doc(db, 'khach_hang', item.id), item);
    }
    ...
    for (const item of donHang) {
      await setDoc(doc(db, 'don_hang', item.id), item);
    }

    // 9. Seed Lịch sử thanh toán (subcollection của đơn hàng)
    ...
    for (const item of lichSuThanhToan) {
      // Lưu như subcollection
      await setDoc(doc(db, 'don_hang', item.don_hang_id, 'lich_su_thanh_toan', item.id), item);
      // Và cũng lưu ở collection riêng để query dễ hơn
      await setDoc(doc(db, 'lich_su_thanh_toan', item.id), item);
    }
    ...
    for (const item of mauThietKe) {
      await setDoc(doc(db, 'mau_thiet_ke', item.id), item);
    }
    ...
    const donThietKe = [
      {
        ...
        doanh_thu: 2500000,
        han_hoan_thanh: Timestamp.fromDate(new Date(now.getTime() + 2 * 86400000)),
      ...
      },
      {
        ...
        doanh_thu: 1500000,
        han_hoan_thanh: Timestamp.fromDate(new Date(now.getTime() + 1 * 86400000)),
        created_at: minutesAgo(4000),
        updated_at: Timestamp.now()
      },
      {
        ...
        doanh_thu: 500000,
        han_hoan_thanh: Timestamp.fromDate(new Date(now.getTime() + 3 * 86400000)),
    ...
    for (const item of donThietKe) {
      await setDoc(doc(db, 'don_thiet_ke', item.id), item);
    }
    ...
    console.log('');
    console.log('🔍 Kiểm tra dữ liệu tại: https://console.firebase.google.com/project/' + firebaseConfig.projectId + '/firestore');
    console.log('');
    process.exit(0);
  } catch (error: any) {
    console.error('');
    ...
    console.error('  2. Firestore đã được bật trong Firebase Console?');
    console.error('  3. Security rules có cho phép đọc/ghi không?');
    console.error('  4. Đã cài đặt dependencies: npm install');
    console.error('');
    if (error.code) {
